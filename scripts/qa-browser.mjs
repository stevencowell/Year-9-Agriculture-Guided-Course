...
async function viewportRun(label,viewport){
  ...
  for(const moduleNumber of [1,10,11,19]){
    ...
    must(await page.locator('a').filter({hasText:'Open larger'}).count()>=3,`${label}: module ${moduleNumber} has Open larger visual routes`);
  ...
  must(await page.getByText(/device-local preparation/i).count()>=1,`${label}: folio storage boundary is visible`);
  ...
  await page.getByRole('link',{name:/Open activity/}).first().click(); await page.waitForLoadState('networkidle');
  ...
  await page.goto(`${base}/plans.html`,{waitUntil:'networkidle'}); must(await page.getByRole('link',{name:/Open larger/}).count()>=1,`${label}: plan route exposes Open larger`); must(await page.getByText(/not a construction plan/i).count()>=1,`${label}: plan boundary is visible`);
  await page.goto(`${base}/assessment.html`,{waitUntil:'networkidle'}); must(await page.getByText(/Teacher to confirm/i).count()>=1,`${label}: assessment conflicts remain visible`);
